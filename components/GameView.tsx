...
export const GameView: React.FC = () => {
    ...
    if (videoAdVisible && videoRef.current && window.fluidPlayer) {
      playerInstance.current = window.fluidPlayer(videoRef.current, {
        ...
        vastOptions: {
          ...
          adFinishedCallback: () => {
            setAdFinished(true);
            addLog("VAST přenos dokončen. Autorizace připravena.", "success");
          }
        }
      });
  ...
  }, [videoAdVisible, isVideoForStart, activeCoinId]);
  ...
  return (
    <div className="flex h-full w-full bg-[#020202] border-t border-[#00f3ff]/10 relative overflow-hidden font-mono text-[#00f3ff]">
      ...
      {/* VAST PLAYER MODAL - Without Countdown */}
      {videoAdVisible && (
          ...
          <div className="w-full max-w-4xl bg-black border-2 border-[#00f3ff]/40 shadow-[0_0_150px_rgba(0,243,255,0.2)] relative overflow-hidden flex flex-col">
            ...
            <div className="p-5 border-b border-[#00f3ff]/20 flex justify-between items-center bg-[#050505]">
               <div className="flex items-center gap-4">
                 ...
                 <span className="text-sm font-black uppercase tracking-[0.25em]">{isVideoForStart ? 'SYSTÉMOVÁ_VERIFIKACE' : 'DATAVÝ_UPLINK'}</span>
               </div>
               {adFinished && (
            ...
            <div className="aspect-video bg-[#010101] relative flex flex-col items-center justify-center overflow-hidden">
               ...
               {/* Overlay - Status message or Confirm Button */}
                  ...
                  {!adFinished ? (
                    <div className="flex flex-col items-center gap-4 bg-black/80 px-12 py-6 border border-[#00f3ff]/20 backdrop-blur-md shadow-2xl pointer-events-auto">
                       <div className="flex items-center gap-6">
                         ...
                         <div className="flex flex-col">
                            <span className="text-[11px] font-black uppercase tracking-[0.3em] text-[#00f3ff]">{!isAdPlaying ? 'INICIALIZACE_UPLINKU' : 'PROBÍHÁ_PŘENOS_DAT'}</span>
                            <span className="text-[9px] text-white/40 uppercase tracking-widest font-bold">STATUS: AUTORIZUJI_PŘIPOJENÍ</span>
                         </div>
                       </div>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center gap-8 animate-in zoom-in slide-in-from-bottom-8 duration-700 pointer-events-auto">
                       <div className="flex items-center gap-3 bg-green-500/20 border-2 border-green-500/50 px-8 py-3 rounded-full shadow-[0_0_20px_rgba(34,197,94,0.3)]">
                          ...
                          <p className="text-xs font-black uppercase tracking-[0.25em] text-green-400">AUTORIZACE_KOMPLETNÍ</p>
                       </div>
                       <button 
                       ...
                       >
                         <span className="relative z-10 font-black">POTVRDIT_A_POKRAČOVAT</span>
            ...
            <div className="p-4 bg-[#050505] flex justify-between items-center border-t border-white/5 px-8">
               <span className="text-[8px] opacity-20 uppercase tracking-[0.4em]">Protocol: HilltopAds_VAST_Secure_Node | Status: {adFinished ? 'VERIFIED' : 'SYNCING'}</span>
               <div className="flex gap-4">
                  <div className="flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${adFinished ? 'bg-green-500' : 'bg-red-500 animate-pulse'}`} />
                    <span className={`text-[8px] uppercase tracking-widest ${adFinished ? 'text-green-500/60' : 'text-red-500/60'}`}>{adFinished ? 'DATOVÝ_TOK_POTVRZEN' : 'ZABEZPEČENÉ_SPOJENÍ'}</span>
                  </div>
               </div>
            </div>
          </div>
