...
import { User, Compass, Package, Zap, Timer, Trophy, Clock, Coins, X, Terminal, Database, PlayCircle, Lock, Eye, Signal, Loader2, Truck, RefreshCw, AlertTriangle, Cpu, HardDrive, Shield } from 'lucide-react';
...
export const GameView: React.FC = () => {
      ...
      const initTimer = setTimeout(() => {
        ...
        try {
          playerInstance.current = window.fluidPlayer(videoRef.current, {
            ...
            vastOptions: {
              allowVPAID: true,
              vpaidMode: 'insecure', // Critical for BST/HilltopAds scripts
              adList: [
                {
                  roll: 'preRoll',
              ...
              adStartedCallback: () => {
                console.log('AD_UPLINK_STARTED');
                addLog("Uplink navázán. Probíhá autorizace...", "info");
              },
              adErrorCallback: (err: any) => {
                console.error("AD_LINK_ERROR", err);
                addLog("Chyba přenosu dat. Restartuji uzel.", "error");
              }
            }
          });
          setAdPlayerInitialized(true);
        } catch (e) {
          console.error("Fluid Player init fail:", e);
        }
      }, 150);
  ...
  return (
    <div className="flex h-full w-full bg-[#020202] border-t border-[#00f3ff]/10 relative overflow-hidden font-mono text-[#00f3ff]">
      ...
      {/* BST VIDEO MODAL - HIGH FIDELITY DESIGN */}
      {videoAdVisible && (
        <div className="fixed inset-0 z-[1000] flex flex-col bg-black animate-in fade-in duration-300">
          ...
          <div className="w-full h-16 bg-black flex items-center justify-between px-8 shrink-0 border-b border-white/5 relative z-50">
            <div className="flex items-center gap-4">
              <div className="w-2.5 h-2.5 rounded-full bg-[#ff00ff] shadow-[0_0_15px_#ff00ff] animate-pulse" />
              <div className="flex flex-col">
                ...
                <span className="text-[8px] text-white/20 uppercase tracking-[0.1em]">Protocol: VPAID_SECURE_TUNNEL</span>
              </div>
            </div>
          ...
          <div className="flex-1 relative bg-[#050505] flex items-center justify-center overflow-hidden">
             ...
             {!adPlayerInitialized && (
               <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm">
                  ...
                  <span className="text-[10px] text-[#00f3ff] uppercase tracking-[0.6em] font-black animate-pulse">Syncing Video Link...</span>
               </div>
             )}
          </div>
             ...
             {videoAdTimer > 0 ? (
                ...
                <div className="flex items-center gap-8 w-full justify-center">
                  ...
                  <button 
                    ...
                    className="absolute right-8 p-3 text-white/20 hover:text-red-500 transition-colors"
                  >
                    <X size={24}/>
                  </button>
                </div>
        ...
        <nav className="flex-1 space-y-3 px-3">
          {([
            ...
            <button
              ...
              className={`w-full flex items-center justify-center md:justify-start gap-4 p-5 transition-all duration-300 rounded-xl ${activeTab === item.id ? 'bg-[#00f3ff]/15 text-[#00f3ff] border-r-4 border-[#00f3ff] shadow-[inset_0_0_20px_rgba(0,243,255,0.1)]' : 'text-white/20 hover:text-white hover:bg-white/5'}`}
            >
              <item.icon size={22} />
      ...
      <main className="flex-1 relative flex flex-col overflow-hidden">
        {activeExpedition ? (
          <div className="flex flex-col h-full animate-in fade-in duration-1000">
            <div className="px-12 py-8 border-b border-white/10 bg-black/95 flex justify-between items-center z-10 shadow-[0_10px_40px_rgba(0,0,0,0.8)]">
              ...
              <div className="flex items-center gap-8">
                ...
                <div className="flex items-center gap-4 bg-[#00f3ff]/5 border border-[#00f3ff]/20 px-8 py-4 rounded shadow-[inset_0_0_20px_rgba(0,243,255,0.2)]">
            ...
            <div className="flex-1 relative bg-[#010101] overflow-hidden">
                  ...
                  {coins.map(coin => (
                    ...
                    >
                       <div className="relative hover:scale-125 transition-transform duration-300">
                          <div className="bg-black border border-[#ff00ff] p-4 flex flex-col items-center gap-2 shadow-[0_0_30px_rgba(255,0,255,0.4)]">
                ...
                {(phase === 'COMPLETED' || phase === 'FAILED') && (
                  <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl">
                    <div className="text-center p-20 border border-[#00f3ff]/30 bg-black/60 shadow-[0_0_100px_rgba(0,243,255,0.2)] max-w-2xl">
                      ...
                      <button onClick={() => setActiveExpedition(false)} className="px-20 py-6 bg-[#00f3ff] text-black font-black uppercase tracking-[0.6em] hover:bg-white transition-all shadow-[0_0_40px_rgba(0,243,255,0.5)]">ODPOJIT</button>
                    </div>
                  </div>
                )}
            </div>

            {/* Terminal Logs Sidebar Replacement for HUD info */}
            <div className="absolute bottom-10 right-10 w-64 h-48 bg-black/80 border border-white/5 p-4 z-40 overflow-y-auto custom-scrollbar text-[10px] space-y-2 pointer-events-none">
              {logs.map(log => (
                <div key={log.id} className={`flex gap-2 opacity-60 ${log.type === 'success' ? 'text-green-400' : log.type === 'error' ? 'text-red-400' : 'text-[#00f3ff]'}`}>
          ...
          <div className="max-w-7xl mx-auto py-24 px-16 w-full h-full overflow-y-auto custom-scrollbar">
            {activeTab === 'profile' && (
              <div className="space-y-16 animate-in slide-in-from-bottom-12 duration-1000">
                <div className="flex flex-col lg:flex-row items-center gap-16 p-16 bg-white/[0.02] border border-white/10 rounded-sm relative overflow-hidden group">
                  <div className="absolute inset-0 bg-[#00f3ff]/5 translate-x-full group-hover:translate-x-0 transition-transform duration-700" />
                  <div className="w-56 h-56 border-2 border-[#00f3ff] p-2 bg-black shadow-[0_0_40px_rgba(0,243,255,0.1)] relative z-10">
            ...
            {activeTab === 'expeditions' && (
              <div className="h-full flex flex-col items-center justify-center space-y-16 animate-in zoom-in duration-700">
                <div className="relative">
                  <Compass size={200} className="text-[#00f3ff] animate-spin-slow drop-shadow-[0_0_50px_rgba(0,243,255,0.4)]" />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <Signal className="text-[#ff00ff] animate-pulse" size={48} />
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="group relative px-24 py-8 border-2 border-[#00f3ff] text-2xl font-black uppercase tracking-[0.6em] hover:bg-[#00f3ff] hover:text-black transition-all shadow-[0_0_60px_rgba(0,243,255,0.3)] overflow-hidden"
                >
                  <span className="relative z-10">AUTORIZOVAT START</span>
                  <div className="absolute inset-0 bg-[#00f3ff] scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500" />
                </button>
              </div>
            )}
            ...
            {activeTab === 'items' && (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-12 animate-in slide-in-from-right-12 duration-1000">
                  {upgrades.map(u => {
                    ...
                    return (
                      <div key={u.id} className={`p-12 border transition-all duration-500 flex flex-col group relative overflow-hidden ${canAfford ? 'border-white/10 bg-white/[0.02] hover:border-[#ff00ff]/40 hover:bg-white/[0.05]' : 'border-white/5 opacity-40 grayscale'}`}>
                        <div className="flex justify-between items-start mb-12 relative z-10">
                          <u.icon className="text-[#ff00ff] group-hover:scale-110 transition-transform" size={48} />
                          <span className="text-4xl font-black text-[#ff00ff] italic">v{u.level}</span>
                        </div>
                        <h4 className="text-2xl font-black text-white uppercase mb-4 relative z-10">{u.name}</h4>
                        <p className="text-xs text-white/40 uppercase mb-12 leading-loose tracking-widest relative z-10">{u.desc}</p>
                        <button 
                          onClick={() => { if(canAfford) { setMikelaReserves(p => p - cost); setUpgrades(prev => prev.map(item => item.id === u.id ? {...item, level: item.level+1} : item)); } }}
                          disabled={!canAfford}
                          className={`w-full py-6 border font-black uppercase tracking-widest transition-all relative z-10 ${canAfford ? 'border-[#ff00ff] text-[#ff00ff] hover:bg-[#ff00ff] hover:text-black shadow-[0_0_20px_rgba(255,0,255,0.2)]' : 'border-white/10 text-white/10'}`}
                        >
      ...
      <style>{`
        ...
        /* Fluid Player Overrides for maximum stability and branding */
        .fluid_video_wrapper {
          width: 100% !important;
